...
//            ->update(['status' => OrderStatusEnum::DELIVERED]);
    ...
    private function createOrderByTrans($snap)
    {
        ...
        try {
            ...
            $daily_id = $this->makeDailyId();
            $order = new OrderModel();
            $order->user_id = $this->uid;
            $mobile = json_decode($snap['snapAddress'])->mobile;
            $order->mobile = $mobile;
            $order->order_no = $orderNo;
            $order->daily_id = $daily_id;
    ...
    public static function makeDailyId()
    {
        $moder = new OrderModel();
        $count = $moder->where(['date'=>date('Y-n-j')])->lock(true)->count();
        return $count+1;
    }
}